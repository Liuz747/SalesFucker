...
async def get_assistant_prompts(
    ...
    """
    try:
        logger.info(f"查询助理提示词: tenant={request.tenant_id}, assistant={assistant_id}")
        
        result = await prompt_handler.get_assistant_prompts(
            assistant_id, request.tenant_id, version
        )
...
async def clone_assistant_prompts(
    ...
    """
    try:
        logger.info(f"克隆助理提示词: tenant={request.tenant_id}, source={source_assistant_id} -> target={assistant_id}")
        
        result = await prompt_handler.clone_assistant_prompts(
            source_assistant_id, assistant_id, request.tenant_id, modify_personality
        )
...
async def get_prompt_history(
    ...
    try:
        ...
        result = await prompt_handler.get_prompt_history(
            assistant_id, request.tenant_id, limit
        )
