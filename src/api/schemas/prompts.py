...
class PromptTemplate(BaseModel):
    ...
    # 提示词内容
    system_prompt: str = Field(description="系统级提示词", min_length=1, max_length=4000)
    user_prompt_template: Optional[str] = Field(
        None, 
        description="用户提示词模板（支持变量替换）",
        max_length=2000
    )
    ...
    usage_instructions: Optional[str] = Field(None, description="使用说明", max_length=1000)
...
class AssistantPromptConfig(BaseModel):
    ...
    context_instructions: Optional[str] = Field(
        None,
        description="上下文处理指令 - 如何利用历史对话和客户信息",
        max_length=1000
    )
    ...
    safety_guidelines: List[str] = Field(
        ...
        description="安全和合规指导原则"
    )
    
    forbidden_topics: List[str] = Field(
        default_factory=list,
        description="禁止讨论的话题列表"
    )
    
    # 品牌和产品信息
    brand_voice: Optional[str] = Field(
        None,
        description="品牌声音定义 - 品牌特色和价值观",
        max_length=500
    )
    
    product_knowledge: Optional[str] = Field(
        None,
        description="产品知识要点 - 重点产品信息和卖点",
        max_length=2000
    )
...
class PromptUpdateRequest(BaseRequest):
    ...
    product_recommendation_prompt: Optional[str] = Field(None, description="产品推荐提示词")
