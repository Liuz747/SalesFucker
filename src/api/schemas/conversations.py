...
class ConversationRequest(BaseRequest):
    ...
    enable_compliance_check: bool = Field(
        True,
        description="是否启用合规性检查"
    )
...
class MessageAttachment(BaseModel):
    ...
    attachment_type: str = Field(description="附件类型", regex="^(image|audio|document)$")
...
class ConversationMessage(BaseModel):
    ...
    agent_responses: Optional[Dict[str, Any]] = Field(None, description="智能体响应详情")
    
    # 分析结果
    sentiment: Optional[Dict[str, Any]] = Field(None, description="情感分析结果")
    intent: Optional[Dict[str, Any]] = Field(None, description="意图分析结果")
    compliance_result: Optional[Dict[str, Any]] = Field(None, description="合规检查结果")
