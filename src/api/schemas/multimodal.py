...
from .responses import SuccessResponse, AsyncTaskResponse
...
class MultimodalRequest(BaseRequest):
    ...
    async_processing: bool = Field(
        False,
        description="是否使用异步处理"
    )
    
    callback_url: Optional[str] = Field(
        None,
        description="异步处理完成后的回调URL"
    )
...
class VoiceProcessingRequest(MultimodalRequest):
    ...
    processing_type: ProcessingType = Field(
        default=ProcessingType.VOICE_TO_TEXT,
        description="固定为语音处理类型"
    )
    
    audio_format: VoiceFormat = Field(
        description="音频格式"
    )
    ...
    duration_seconds: Optional[float] = Field(
        ...
        ge=0.1,
        le=300,
        description="音频时长（秒），最长5分钟"
    )
    
    enable_emotion_detection: bool = Field(
        False,
        description="是否启用情感检测"
    )
...
class ImageAnalysisRequest(MultimodalRequest):
    ...
    detect_products: bool = Field(
        True,
        description="是否检测化妆品产品"
    )
    
    analyze_skin_condition: bool = Field(
        False,
        description="是否分析肌肤状况"
    )
...
class ImageAnalysisResult(BaseModel):
    ...
    skin_analysis: Optional[Dict[str, Any]] = Field(
        None,
        description="肌肤分析结果"
    )
