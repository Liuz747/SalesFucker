...
class AgentTestRequest(BaseRequest):
    ...
    agent_id: Optional[str] = Field(
        None,
        description="要测试的智能体ID，如果不提供则自动选择"
    )
...
class AgentConfigUpdateRequest(BaseRequest):
    ...
    merge_config: bool = Field(
        True,
        description="是否合并配置（False表示完全替换）"
    )
    
    restart_agent: bool = Field(
        False,
        description="是否重启智能体以应用配置"
    )
