...
class MultiLLMBaseAgent(BaseAgent):
    ...
    def _update_routing_strategy(self, strategy: RoutingStrategy):
        """更新路由策略"""
        self.routing_strategy = strategy
        self.logger.info(f"智能体路由策略更新: {self.agent_id} -> {strategy.value}")
    
    def _update_llm_preferences(self, preferences: Dict[str, Any]):
        """更新LLM偏好设置"""
    ...
    # 集成的LLM接口功能 (原 LLMInterface 类)
    async def _get_llm_client(self) -> MultiLLMClient:
        """获取多LLM客户端实例"""
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        base_status["data"]["config"] = self._get_config_summary()
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        # 添加LLM相关健康指标
        llm_health = self._get_health_metrics()
