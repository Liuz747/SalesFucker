...
from api.dependencies.agents import get_agent_registry_service, validate_agent_id
...
async def test_compliance_agent_legacy(
    ...
    """
    service_context = getattr(request.state, 'service_context', None)
    actual_tenant_id = service_context.get('tenant_id') if service_context else tenant_id
    
    logger.warning(
        f"使用已废弃的不安全端点: /tenant/{tenant_id}/compliance/test, "
f"实际租户: {actual_tenant_id}"
    )
    
    # 在新架构下，middleware已验证租户身份
    
    try:
        agent_id = f"compliance_{tenant_id}"
        test_request = AgentTestRequest(test_message=test_message, tenant_id=tenant_id)
...
f"实际租户: {actual_tenant_id}"
    ...
    try:
        agent_id = f"sentiment_{tenant_id}"
        test_request = AgentTestRequest(test_message=test_message, tenant_id=tenant_id)
